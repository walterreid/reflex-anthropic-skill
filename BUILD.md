...
| 2026-04-01 | Three-layer copilot fix: intent framing + externalization + registry signals | Layer 1: user-intent/mod-analogy framing in PERSONA.md ("the user loaded this persona, honor that"). Layer 2: startup externalization — copilot commits out loud to using modules, creating an in-context contract harder to silently violate. Layer 3: all 6 formatter PARAMS.json descriptions updated to signal "use this instead of native tools" — visible in the registry the copilot receives at load time. |
