...
Personas are a persistent conversational layer over module dispatch. A user types `reflex persona walt` and gets a thinking partner who silently orchestrates modules — no chain syntax, no module names, just conversation.

**Why personas are not modules:** Modules are bounded (input → output → done) and composable (`+` chains, DEPENDS.json). A persona is persistent (stays active across turns) and unbounded (no defined output shape). If a persona lived in `modules/`, it would be chainable — `websearch+walt` would be valid syntax, producing incoherent behavior. The parallel `personas/` directory protects this invariant at the type level: everything in `modules/` is composable, everything in `personas/` is persistent. No flags, no exclusion lists, no special cases in dispatch.py.
...
| 8.1 | First persona: `walt` with PERSONA.md, PARAMS.json, TRIGGERS.json, STYLE.json | [x] Installed, not yet live-tested |
| 8.2 | Live test: `reflex persona walt` in a fresh conversation — validate persistence, module invocation, trigger recognition, style adaptation | [~] First test: zero module invocations. Second iteration: user-intent framing + mod analogy added. Third iteration: startup externalization (walt commits out loud to using modules), formatter PARAMS.json descriptions updated to signal value to walt registry. Needs re-test with all three layers. |
| 8.3 | Test persona/module isolation — confirm walt does not appear in module registry or chain composition | [ ] Verify via dispatch.py --list and plan's registry injection |
...
| 2026-04-01 | Persona system added as parallel architecture (not as modules) | Personas violate the module composition contract — they're persistent and unbounded, modules are bounded and composable. Putting a persona in `modules/` would make it chainable (`websearch+walt`), producing incoherent behavior. The parallel `personas/` directory protects the composition invariant at the type level. dispatch.py untouched. |
| 2026-04-01 | Walt PERSONA.md rewritten: suggestive → directive module invocation | First live test showed walt used zero reflex modules despite having the full registry injected. LLM chose native tools (WebSearch, Write) as path of least resistance. Fix: mandatory module rule ("MUST use reflex modules for any work product"), concrete dispatch examples for 7 common patterns, MUST directives in all TRIGGERS.json notes. |
| 2026-04-01 | Three-layer walt fix: intent framing + externalization + registry signals | Layer 1: user-intent/mod-analogy framing in PERSONA.md ("the user loaded this persona, honor that"). Layer 2: startup externalization — walt commits out loud to using modules, creating an in-context contract harder to silently violate. Layer 3: all 6 formatter PARAMS.json descriptions updated to signal "use this instead of native tools" — visible in the registry walt receives at load time. |
